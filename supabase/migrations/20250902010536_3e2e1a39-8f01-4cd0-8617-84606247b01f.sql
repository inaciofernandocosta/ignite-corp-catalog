-- Remover registro duplicado da Mariana (manter o mais antigo)
DELETE FROM inscricoes_mentoria 
WHERE id = 'ea31e2d6-0fee-43db-83f2-c11bd62bc360';

-- Corrigir função para criar contas de auth (sem gen_salt)
...
BEGIN
  ...
  LIMIT 1; -- Pegar apenas um registro em caso de duplicatas
  ...
  -- Criar usuário no auth.users (senha será resetada via email)
  ...
  RETURN 'Conta criada com sucesso para ' || user_email || '. Use reset de senha para definir uma nova senha.';
